...
if (window.Sys && Sys.Application) {
    Sys.Application.add_load(cargarProducto);
} else {
...
function validarProducto() {
    ...
    if (!nombre || !cantidad || !precio || proveedor === "0") {
        mostrarModalProducto("Faltan campos", "Complete todos los campos obligatorios.");
        return false;
    }

    if (!/^[A-Za-zÁÉÍÓÚáéíóúÑñ0-9\s\-_.]+$/.test(nombre)) {
        mostrarModalProducto("Nombre inválido", "El nombre solo debe contener letras, números y espacios.");
        return false;
    }

    if (!/^[0-9]+$/.test(cantidad) || parseInt(cantidad) < 0) {
        mostrarModalProducto("Cantidad inválida", "La cantidad debe ser un número entero válido.");
        return false;
    }

    if (!/^[0-9]+(\.[0-9]{1,2})?$/.test(precio) || parseFloat(precio) <= 0) {
        mostrarModalProducto("Precio inválido", "El precio debe ser mayor a cero y máximo con 2 decimales.");
        return false;
    }
    ...
    if (!previewContainer || previewContainer.getElementsByTagName("img").length === 0) {
        mostrarModalProducto("Imágenes requeridas", "Primero debe seleccionar y previsualizar al menos una imagen.");
        return false;
    }
...
function validarArchivoProductoCsv() {
    ...
    if (!archivo || archivo.files.length === 0) {
        mostrarModalProducto("Archivo requerido", "Debe seleccionar un archivo CSV de productos.");
        return false;
    }
    ...
    if (!nombre.endsWith(".csv")) {
        mostrarModalProducto("Archivo inválido", "Solo se permite archivo .csv.");
        archivo.value = "";
        return false;
    }
    ...
    if (file.size > maxBytes) {
        mostrarModalProducto("Archivo muy pesado", "El archivo no debe superar " + maxMb + " MB.");
        archivo.value = "";
        return false;
    }
...
function validarFotosProducto() {
    ...
    if (!fotos || fotos.files.length === 0) {
        mostrarModalProducto("Imágenes requeridas", "Seleccione al menos una imagen.");
        return false;
    }
    ...
    for (let i = 0; i < fotos.files.length; i++) {
        ...
        if (!extensionValida) {
            mostrarModalProducto("Archivo no permitido", "Solo se permiten imágenes JPG o PNG.");
            fotos.value = "";
            return false;
        }

        if (archivo.size > maximo) {
            mostrarModalProducto("Imagen muy pesada", "Cada imagen no debe superar los 5 MB.");
            fotos.value = "";
            return false;
        }
    }
